...
export const ASSESSMENT_CATALOG: Record<string, AssessmentMeta> = {
  // 平衡与下肢（双侧，左右对比判短板）
  single_leg_stand: { capacityId: 'balance', domain: 'physical_base', label: '单腿站平衡', unit: '秒', kind: 'bilateral', group: '平衡与下肢' },
  sit_to_stand: { capacityId: 'joint_control', domain: 'physical_base', label: '单腿坐站', unit: '次', kind: 'bilateral', group: '平衡与下肢' },
  // 力量（标量，越高越好）
  pushup: { capacityId: 'strength', domain: 'physical_base', label: '标准俯卧撑', unit: '次', kind: 'scalar', group: '力量', threshold: { min: 5 } },
  // 核心（标量）
  side_plank: { capacityId: 'joint_control', domain: 'physical_base', label: '侧平板支撑', unit: '秒', kind: 'scalar', group: '核心', threshold: { min: 20 } },
  // 活动度（标量）
  ankle_dorsiflexion: { capacityId: 'mobility', domain: 'physical_base', label: '踝背屈（膝触墙）', unit: 'cm', kind: 'scalar', group: '活动度', threshold: { min: 8 } },
  // 心肺（标量，越低越好：同等负荷下 RPE 越低越省力）
  walk_rpe: { capacityId: 'aerobic', domain: 'physical_base', label: '30 分钟快走 RPE', unit: '1–10', kind: 'scalar', group: '心肺', threshold: { max: 6 } },
};
